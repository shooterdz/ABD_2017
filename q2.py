...
if __name__ == "__main__":
	...
	if(os.path.exists("./res_q2")):
		shutil.rmtree("./res_q2")
	...
	lignes = lignes.map(lambda x: x.split(',')).map(lambda a: (a[0], 1)).reduceByKey(lambda a,b:int(int(a) + int(b))).sortBy(lambda a : a[1], ascending = False)


	lignes.saveAsTextFile('res_q2')
